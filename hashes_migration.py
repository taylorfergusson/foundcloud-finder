...
try:
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS song_hashes (
            ...
            song_urls TEXT[] -- Store multiple song URLs
        );
    """)
    print("Committing")
    conn.commit()
    ...
    # Insert data
    insert_query = """
        INSERT INTO song_hashes (hash, song_urls) 
        VALUES (%s, %s) 
        ON CONFLICT (hash) 
        DO UPDATE 
        SET song_urls = 
            CASE 
                WHEN NOT song_hashes.song_urls @> EXCLUDED.song_urls THEN 
                    song_hashes.song_urls || EXCLUDED.song_urls
                ELSE
                    song_hashes.song_urls
            END;
        """
    data = []
